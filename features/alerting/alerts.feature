...
Feature: Alerts
  ...
  # Product Question:
  # Q: Just to confirm, will the first release deliver:
  #    ___ a packaged, custom or open source alerting system
  #    ___ a reference implementation of an existing alerting solution for example purposes
  #    ___ hooks for alerting systems beyond what exists to facilitate kibana and grafana UIs
  #    ___ documentation about alerting in general
  ...
  Scenario: "Watermarks" aka different thresholds for alarms to different people
    * ie. Volume has an owner, so send alert to them at lower threshold, send to general admin at higher threshold or if no action is taken
