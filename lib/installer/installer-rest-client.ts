...
export class InstallerRestClient implements IInstallerRestClient {
    ...
    createAutheticatedSession(installerProtocolHostAndOptionalPort:string, username:string, password:string):IFuture<IInstallerRestSession> {
        const restClientAsPromised = this.rest.newRestClient(installerProtocolHostAndOptionalPort);
        const loginBody = {
            username: username,
            password: password
        };
        return restClientAsPromised.postFormEncodedBody(this.configuration.installerLoginPath, loginBody)
            ...
            .then(apiResponse => {
                return this.installer.newInstallerRestSession(
                    ...
                    this.typedJSON.newJSONObject(apiResponse.jsonHash)
                );
            });
    }
}