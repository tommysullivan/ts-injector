...
module.exports = function(configJSON) {
    ...
    return {
        getFlattenedClusterConfig: function(clusterUnderTestId) {
            var config = _.findWhere(configJSON.testClusters, {id: clusterUnderTestId});
            if(config==null) throw new Error(`Could not find config with id ${clusterUnderTestId}`);
            return config.inheritsFrom ? _.extend(this.getFlattenedClusterConfig(config.inheritsFrom), config) : config;
        },
        ...
        newClusterUnderTest: function(clusterUnderTestId, repositories) {
            var clusterUnderTestConfig = this.getFlattenedClusterConfig(clusterUnderTestId);
