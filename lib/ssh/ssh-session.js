"use strict";
var SSHSession = (function () {
    function SSHSession(nodemiralSession, promiseFactory, nodeWrapperFactory, collections, api, host, writeCommandsToStdout, fileSystem, scp2Module, username, password, temporaryStorageLocation, uuidGenerator, path, errors) {
        this.nodemiralSession = nodemiralSession;
        this.promiseFactory = promiseFactory;
        this.nodeWrapperFactory = nodeWrapperFactory;
        this.collections = collections;
        this.api = api;
        this.host = host;
        this.writeCommandsToStdout = writeCommandsToStdout;
        this.fileSystem = fileSystem;
        this.scp2Module = scp2Module;
        this.username = username;
        this.password = password;
        this.temporaryStorageLocation = temporaryStorageLocation;
        this.uuidGenerator = uuidGenerator;
        this.path = path;
        this.errors = errors;
    }
    SSHSession.prototype.executeCommands = function (commands) {
        var _this = this;
        return this.promiseFactory.newPromise(function (resolve, reject) {
            var results = _this.collections.newEmptyList();
            var executeNextCommand = function (commandsToExecute) {
                var commandToExecute = commandsToExecute.first();
                var remainingCommands = commandsToExecute.rest();
                if (commandToExecute == null)
                    return reject(new Error('Attempted to execute null command'));
                _this.executeCommand(commandToExecute)
                    .then(function (result) {
                    results.push(result);
                    remainingCommands.notEmpty()
                        ? executeNextCommand(remainingCommands)
                        : resolve(results);
                })
                    .catch(function (sshError) {
                    results.push(sshError.sshResult);
                    reject(_this.api.newSSHMultiCommandError(results));
                });
            };
            executeNextCommand(commands.map(function (i) { return i; }));
        });
    };
    SSHSession.prototype.executeCommand = function (command) {
        var _this = this;
        if (this.writeCommandsToStdout)
            console.log(command);
        return this.promiseFactory.newPromise(function (resolve, reject) {
            _this.nodemiralSession.onError(function (error) {
                reject(_this.api.newSSHError(error, null));
            });
            _this.nodemiralSession.execute(command, function (err, code, logs) {
                var processResult = _this.nodeWrapperFactory.newProcessResult(command, code, _this.collections.newList(logs.stdout.split("\n")), _this.collections.newList(logs.stderr.split("\n")), err);
                var result = _this.api.newSSHResult(_this.host, processResult);
                if (err)
                    reject(_this.api.newSSHError(err, result));
                else if (code != 0)
                    reject(_this.api.newSSHError("Process exited with nonzero exit code: " + code, result));
                else
                    resolve(result);
            });
        });
    };
    SSHSession.prototype.upload = function (localPath, remotePath) {
        var _this = this;
        return this.promiseFactory.newPromise(function (resolve, reject) {
            _this.scp2Module.scp(localPath, { path: remotePath }, _this.newKeyboardInteractiveClient(), function (err) {
                if (err)
                    reject(err);
                else
                    resolve(null);
            });
        });
    };
    SSHSession.prototype.download = function (remotePath, localPath) {
        var _this = this;
        return this.promiseFactory.newPromise(function (resolve, reject) {
            var options = {
                host: _this.host,
                path: remotePath
            };
            _this.scp2Module.scp(options, localPath, _this.newKeyboardInteractiveClient(), function (error) {
                if (error) {
                    try {
                        throw _this.errors.newErrorWithCause(error, "remote path: " + remotePath + ", local path: " + localPath);
                    }
                    catch (e) {
                        reject(e);
                    }
                }
                else
                    resolve(null);
            });
        });
    };
    SSHSession.prototype.write = function (fileContent, destinationPath) {
        var _this = this;
        return this.promiseFactory.newPromise(function (resolve, reject) {
            console.log("writing file content to \"" + destinationPath + "\"", fileContent);
            _this.newKeyboardInteractiveClient().write({
                destination: destinationPath,
                content: _this.nodeWrapperFactory.newStringBuffer(fileContent)
            }, function (err) {
                if (err)
                    reject(err);
                else
                    resolve(null);
            });
        });
    };
    SSHSession.prototype.read = function (remotePath) {
        var _this = this;
        var localPath = this.path.join(this.temporaryStorageLocation, this.uuidGenerator.v4());
        return this.download(remotePath, localPath)
            .then(function (_) { return _this.fileSystem.readFile(localPath); })
            .then(function (fileContent) { return _this.fileSystem.delete(localPath).then(function () { return fileContent.toString(); }); });
    };
    SSHSession.prototype.newKeyboardInteractiveClient = function () {
        var _this = this;
        var client = new this.scp2Module.Client({
            host: this.host,
            username: this.username,
            password: this.password,
            tryKeyboard: true
        });
        client.on('keyboard-interactive', function (name, instr, lang, prompts, cb) {
            cb([_this.password]);
        });
        return client;
    };
    return SSHSession;
}());
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = SSHSession;
//# sourceMappingURL=ssh-session.js.map