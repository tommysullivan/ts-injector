module.exports = function(api, express, path, http, fs, fullyQualifiedResultsPath, bodyParser, jiraRestClient, jiraUsername, jiraPassword, _, fullyQualifiedConfigsPath, fullyQualifiedCLIInvocationsPath) {
    var testResultsWebPath = '/test-results/';
    var testResultRoute = `${testResultsWebPath}:resultId`;
    var testConfigRoute = '/test-configs/:configId';
    var testCliInvocationsRoute = '/test-cli-invocations/:cliDetailId';
    return {
        startServer: function(hostname, httpPort) {
            return api.newPromise((resolve, reject) => {
                ...
                try {
                    ...
                    expressApplication.use(bodyParser.json({limit: '5mb'}));
                    ...
                    expressApplication.get(testResultsWebPath, function(req, res) {
                        var maxResults = 20;
                        var fileNames = fs.readdirSync(fullyQualifiedResultsPath).filter(f=>f.indexOf('.json')>-1);
                        ...
                        fileDescriptors = fileDescriptors.sort((f1, f2) => f1.modifiedTime - f2.modifiedTime);
                        fileDescriptors = fileDescriptors.slice(Math.max(fileDescriptors.length - maxResults, 0));
                    ...
                    var getTestConfigFilePathFromId = (configId) => path.joinFilePath(fullyQualifiedConfigsPath, `${configId}.json`);
                    var getTestCliInvocationFilePathFromId = (cliDetailId) => path.joinFilePath(fullyQualifiedCLIInvocationsPath, `${cliDetailId}.txt`);
                    var getTestResultFilePathFromHttpRequest = (httpRequest) => getTestResultFilePathFromId(httpRequest.params.resultId);
                    var getTestConfigFilePathFromHttpRequest = (httpRequest) => getTestConfigFilePathFromId(httpRequest.params.configId);
                    var getTestCliInvocationsFilePathFromHttpRequest = (httpRequest) => getTestCliInvocationFilePathFromId(httpRequest.params.cliDetailId);
                    ...
                    expressApplication.get(testResultRoute, function(httpRequest, httpResponse) {
                        if(httpRequest.accepts('text/html'))
                            httpResponse.redirect(`/#/test-result-viewer/${httpRequest.params.resultId}`);
                        else
                            fs.createReadStream(getTestResultFilePathFromHttpRequest(httpRequest)).pipe(httpResponse);
                    });
                    expressApplication.get(testConfigRoute, function(httpRequest, httpResponse) {
                        fs.createReadStream(getTestConfigFilePathFromHttpRequest(httpRequest)).pipe(httpResponse);
                    });
                    expressApplication.get(testCliInvocationsRoute, function(httpRequest, httpResponse) {
                        fs.createReadStream(getTestCliInvocationsFilePathFromHttpRequest(httpRequest)).pipe(httpResponse);
                    });
                    expressApplication.put(testResultRoute, function(httpRequest, httpResponse) {
                        fs.writeFile(getTestResultFilePathFromHttpRequest(httpRequest), JSON.stringify(httpRequest.body, null, 3), function (error) {
