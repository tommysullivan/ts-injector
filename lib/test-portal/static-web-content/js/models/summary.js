angular.module('testPortalApp').factory('summaryModel', function() {
    var summaryOutputStatuses = ['passed','failed','notExecuted','pending','total','empty'];
    return {
        ...
        aggregateStatus: (summary) => {
            return summary.passed + summary.empty == summary.total
                    ...
                    : summary.failed > 0
                        ? 'failed'
                        : summary.pending > 0 || summary.passed > 0
                            ? 'pending'
                            : 'notExecuted';
        }
    }
