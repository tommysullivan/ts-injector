angular.module('testPortalApp').factory('TestResult', function(summaryModel, featureModel) {
    function TestResult(featureModels, name, href, modifiedTime, contentType, testResultJSON) {
        return {
            ...
            startTime: testResultJSON.cucumberTestResult ? testResultJSON.cucumberTestResult.startTime : null,
            endTime: testResultJSON.cucumberTestResult ? testResultJSON.cucumberTestResult.endTime : null,
            clusterId: () => {
    ...
    return {
        fromJSON: (testResultJSON, name, href, modifiedTime) => {
            ...
            switch(contentType) {
                case 'vnd/mapr.test-portal.cluster-test-result+json;v=2.0.0':
                    featuresJSONArray = testResultJSON.cucumberTestResult.resultAcquisitionError
                        ? []
                        : testResultJSON.cucumberTestResult.cucumberFeatureResults;
                    break;
                case 'vnd/mapr.test-portal.cluster-test-result+json;v=1.0.0':
