angular.module('testPortalControllers').controller('TestResultsExplorerController', function (
    $scope, $http, $q, $location, $anchorScroll, TestResult, statusesModel, pieChart, summaryModel) {
        ...
        function bindPieChart() {
            $scope.summaryPieChart = pieChart.forSummary(
                summaryModel.aggregateSummary(featureSummaries(), 'features')
            );
        }
...
//            //$scope.testResults = originaltestResults;
//
//            //lets apply the query to the testResults. We will set $scope.
//            var query = {
...
//            //query based on tags
//
//            $scope.testResults = originalTestResults.filter(query);
