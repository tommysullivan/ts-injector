angular.module('testPortalApp').controller('TestResultsExplorerController', function ($scope, $http, $q, $location, $anchorScroll, statusesModel, summaryModel, pieChart, TestResult) {
    ...
    $http.get(testResultsPath).then(response => {
        ...
        var testResultRequests = testResultLinksJSON.map((testResultLinkJSON, i) => {
            ...
            var desiredContentType = 'vnd/mapr.test-portal.cluster-test-result-summary+json;v=1.0.0';
            return $http.get(url, {headers: {Accept: desiredContentType}}).then(testResultResponse => {
