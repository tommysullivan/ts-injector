angular.module('testPortalControllers').controller('TestResultController', function (
    ...
    function applyQuery() {
        ...
        var soughtTagsWithNoMatch = soughtTagNames()
            ...
            .map(tag.fromName);
        $scope.flattenedClusterConfig = $scope.testResult.testResultJSON().flattenedClusterConfig;
        $scope.versionGraph = $scope.testResult.testResultJSON().versionGraph;
        $scope.configJSON = $scope.testResult.testResultJSON().configJSON;
