angular.module('testPortalApp').controller('TestResultController', function (
    ...
    function applyQuery() {
        ...
        $scope.logs = $scope.unfilteredTestResult.logs();
        $scope.logCheckboxes = {};
        ...
        try {
            $scope.testRunConsoleOutput = $scope.testResult.
            $scope.configJSON = $scope.testResult.configJSON();
            $scope.testRunConsoleOutput = $scope.testResult.toJSON().cucumberTestResult.processResult.stdoutLines.map(l=>l.replace('[32m','').replace('[36m', '').replace('[39m', '').replace('[31m','').replace('[33m','')).join("\n");
        }
        catch(e) {
            console.log('Warning, could not render raw config JSON or raw console output');
        }
        $scope.testResultJSON = JSON.stringify($scope.testResult.toJSON(), null, 3);
