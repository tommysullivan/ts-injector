module.exports = function(api, clusterUnderTestConfig, clusterNodes, _) {
    ...
    return {
        ...
        revertToState: (desiredStateName) => {
            return multiNodePromise(n=>n.revertToState(desiredStateName));
        },
        deleteStateIfExists: (stateName) => {
            return multiNodePromise(n=>n.deleteStateIfExists(stateName));
        },
        verifyMapRNotInstalled: () => {
            return multiNodePromise(n=>n.verifyMapRNotInstalled());
        },
        ...
        executeShellCommandsOnEachNode: function(commands) {
            multiNodePromise(n=>n.executeShellCommands(commands));
        },
        captureStateAsSnapshot: function(snapshotName) {
            multiNodePromise(n=>n.captureStateAsSnapshot(snapshotName));
        },
        snapshotInfo: function() {
            multiNodePromise(n=>n.snapshotInfo());
        },
